  ...
  !(function (P) {
      ...
      (_createClass(t, [
        {
          ...
          value: function (t) {
            ...
            var e = P(
              "<canvas id='mouseCanvas' style='position: fixed; left: 0px; top: 0px; z-index: 10011;pointer-events:none;'></canvas>"
            );
            P('body').append(e);
            ...
            1 <= i &&
              ...
              11 == i && this.mouseType2(),
              12 == i && this.mouseType3();
          },
        },
        {
          key: 'mouseType1',
          value: function (t, e) {
            var n,
              ...
              a,
              h,
              i = document.getElementById('mouseCanvas'),
              s = i.getContext('2d'),
              r = P('#mouseCanvas'),
              l = [],
              c = { n: 100, c: 222, bc: '#fff', r: 0.9, o: 0.05, a: 1, s: 20 },
              d = 0,
              f = 0,
              u = 0,
              y = 0,
            ...
            P(window).mousemove(function (t) {
              (a = t.clientX - r.offset().left),
                (h = t.clientY - r.offset().top),
                4 == M &&
                (Math.random() <= 0.5
                ...
                if (1 == M) {
                  ...
                  for (var e = 0; e < l.length; e++)
                    (s.globalAlpha = l[e].o),
                      (s.fillStyle = m),
                      s.beginPath(),
                      s.arc(l[e].x, l[e].y, l[e].r, 0, 2 * Math.PI),
                      s.closePath(),
                      s.fill(),
                      (l[e].r += c.r),
                      (l[e].o -= c.o),
                  ...
                  for (s.clearRect(0, 0, n, o), e = 0; e < l.length; e++)
                    (s.globalAlpha = l[e].o),
                      (s.fillStyle = m),
                      s.beginPath(),
                      (l[e].r = 10),
                  ...
                  for (s.clearRect(0, 0, n, o), e = 0; e < l.length; e++)
                    (s.globalAlpha = l[e].o),
                      ...
                      (l[e].o -= c.o / 2),
                      (l[e].r = 10),
                      s.arc(l[e].wx, l[e].wy, l[e].r, 0, 2 * Math.PI),
                      s.closePath(),
                      s.fill(),
                      (s.shadowBlur = 0),
                      l[e].o <= 0 && (l.splice(e, 1), e--);
                  ...
                  for (
                    ...
                    (s.globalAlpha = l[e].o),
                      ...
                      (l[e].x += (a - l[e].x) / 10),
                      (l[e].y += (h - l[e].y) / 10),
                      s.arc(l[e].x, l[e].y, 1, 0, 2 * Math.PI),
                      s.closePath(),
                      s.fill(),
                      (l[e].o -= c.o),
                      l[e].o <= 0 &&
                    ...
                    for (var i = h - 60; i < h + 60; i += 4)
                      Math.sqrt(Math.pow(a - e, 2) + Math.pow(h - i, 2)) <=
                        ...
                        ((s.globalAlpha =
                          ...
                          Math.sqrt(Math.pow(a - e, 2) + Math.pow(h - i, 2)) /
                          60),
                          Math.random() < 0.2 && s.fillRect(e, i, 3, 3));
                else if (9 == M)
                  for (
                    s.clearRect(0, 0, n, o),
                    s.fillStyle = m,
        ...
        },
        {
          ...
          value: function () {
            ...
            function p(t) {
              1 == t.touches.length &&
                ((d = t.touches[0].clientX - 0.5 * (window.innerWidth - h)),
                  (f = t.touches[0].clientY - 0.5 * (window.innerHeight - s)));
            }
            function v(t) {
              1 == t.touches.length &&
                ((d = t.touches[0].clientX - 0.5 * (window.innerWidth - h)),
            ...
            function x() {
              ...
              for (var t = 0, e = a.length; t < e; t++) {
                var i = a[t],
                  n = { x: i.position.x, y: i.position.y };
                (i.offset.x += i.speed),
                  (i.offset.y += i.speed),
            ...
            (t = document.getElementById('mouseCanvas')) &&
              t.getContext &&
              ((o = t.getContext('2d')),
        ...
        },
        {
          ...
          value: function () {
            var t = P(
              "<div id='mouseDiv' style='position: fixed;width: 100%;height: " +
              P(window).height() +
              "px; left: 0px; top: 0px; z-index: 10011;pointer-events:none;'></div>"
            );
            P('body').append(t),
              new ((function () {
                function i(t) {
                ...
                t.prototype = {
                  ...
                  start: function () {
                    ...
                    for (var n = 0; n < 360; n += 20)
                      for (var o = t, a = 10; a < 35; a += 1) {
                        var h = i(n, a).x,
                          s = i(n, a).y;
                        (r[l++] = e = new c(o, h, s)), (o = e);
                      }
                    setInterval(function () {
                      for (var t = 0; t < l; t++) r[t].run();
                    }, 16);
                  },
                };
                var c = function (t, e, i) {
                  ...
                  (this.css = n.style),
                    ...
                    document.getElementById('mouseDiv').appendChild(n),
                    (this.ddx = 0),
                    (this.ddy = 0),
                    (this.PX = 0),
                    (this.PY = 0),
                ...
                animR: 2,
                fn: function (t, e) {
                  return { x: (e / 4) * Math.cos(t), y: (e / 4) * Math.sin(t) };
                },
              });
          },
        },
    ...
    function t() {
      _classCallCheck(this, t),
        (this.defaluts = { type: 1, color: !1 }),
        (this.version = '0.0.1');
    }
  })($);
  let flag = _getData('mymouse') || 'y';
  parent.window === window && flag === 'y' && $.myMouse({ type: 11 });
})($);
...
~(function () {
  ...
  let timer, sakuraNum, limitTimes, limitArray;
  function init() {
    timer = null;
    // 樱花数量
    sakuraNum = 41;
    // 限制每片樱花出现次数, -1不做限制,无限循环
    limitTimes = 2;
    // 统计所有樱花出现次数数组
    limitArray = new Array(sakuraNum).fill(limitTimes);
  }
  ...
  Sakura.prototype.update = function () {
    ...
    if (
      ...
      // 樱花出现次数限制
      if (limitArray[this.idx] == 0) return;
  ...
  function startSakura() {
    ...
    canvas.setAttribute(
      'style',
      `position: fixed;left: 0;top: 0;pointer-events: none;z-index: ${LevelObj.hua};`
    );
    canvas.setAttribute('id', 'canvas_sakura');
    document.body.appendChild(canvas);
    cxt = canvas.getContext('2d');
    let sakuraList = new SakuraList(); //生成储存樱花实例数组
    for (let i = 0; i < sakuraNum; i++) {
      ...
      sakuraList.push(sakura); //放入樱花数组
    }
    // 递归 更新所有樱花位置 绘制修改后的樱花
    let flag = true;
    (function next() {
      timer = requestAnimationFrame(function () {
        ...
        sakuraList.draw(cxt);
        //所有樱花次数都为0，关闭樱花
        if (limitArray.every((item) => item == 0) && flag === true) {
          flag = false;
  ...
  // 加载完樱花图后开启
  img.onload = function () {
    let flag = _getData('hua') || 'y';
    parent.window === window && flag === 'y' && startSakura();
  };
  // 关闭樱花特效
  function stop() {
