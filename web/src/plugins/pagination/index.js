...
export function pagination(opations) {
  let { pageNo, pageSize, total, continuous = 5 } = opations;
  pageNo = +pageNo;
  pageSize = +pageSize;
  total = +total;
  ...
  if (totalPage > continuous) {
    ...
    endPage > totalPage
      ? ((endPage = totalPage), (startPage = totalPage - continuous + 1))
      : null;
  } else {
    startPage = 1;
    endPage = totalPage;
  }
  let str = `<section id="paginationBox">`;
  ...
  for (let i = startPage; i <= endPage; i++) {
    str += `<button cursor data-flag="${i}" class="${i == pageNo ? 'active' : ''
      }">${i}</button>`;
  }
  if (totalPage > continuous) {
    str += `${endPage < totalPage - 2
      ? `<button cursor data-flag="${endPage + 1}">...</button>`
      : ''
      }
            ${endPage == totalPage - 2
        ? `<button cursor data-flag="${totalPage - 1}">${totalPage - 1
        }</button>`
        : ''
      }
            ${endPage < totalPage
        ? `<button cursor data-flag="${totalPage}">${totalPage}</button>`
        : ''
      }`;
  }
  str += `${pageNo < totalPage ? '<button cursor data-flag="next">下一页</button>' : ''
    }`;
  str += `<span>一共 ${total} 条,</span><span>跳转到</span>
